  ...
  Future<void> submitVerification(PsaVerification verification) async {
    try {
      ...
      if (true) { // Always log for debugging
        print('📝 Submitting verification to Firestore:');
        print('   PSA ID: ${data['psa_id']}');
        print('   Business: ${data['business_name']}');
        print('   Status: ${data['status']}');
        print('   Business License URL: ${data['business_license_url'] ?? "NULL"}');
        print('   Tax ID Document URL: ${data['tax_id_document_url'] ?? "NULL"}');
        print('   National ID URL: ${data['national_id_url'] ?? "NULL"}');
        print('   Trade License URL: ${data['trade_license_url'] ?? "NULL"}');
      }
      
      await _firestore.collection(_collection).add(data);
      print('✅ Verification submitted successfully to Firestore');
    } catch (e) {
