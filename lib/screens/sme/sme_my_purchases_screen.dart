  ...
  Widget _buildOrdersList(
    ...
    return StreamBuilder<List<app_order.Order>>(
      ...
      builder: (context, snapshot) {
        ...
        if (kDebugMode) {
          debugPrint(
            '📊 SME My Purchases - Connection: ${snapshot.connectionState}',
          );
          debugPrint('📊 SME My Purchases - Has Error: ${snapshot.hasError}');
          debugPrint(
            '📊 SME My Purchases - Data: ${snapshot.data?.length ?? 0} orders',
          );
          debugPrint(
            '📊 SME My Purchases - Status Filter: ${statusFilter.map((s) => s.toString()).join(", ")}',
          );
        }
