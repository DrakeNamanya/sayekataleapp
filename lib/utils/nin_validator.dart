...
class NINValidator {
  ...
  static bool namesMatch(String? name1, String? name2) {
    ...
    final normalized1 = name1.trim().toLowerCase().replaceAll(RegExp(r'\s+'), ' ');
    final normalized2 = name2.trim().toLowerCase().replaceAll(RegExp(r'\s+'), ' ');
...
class VerificationResult {
  ...
  bool get isFullyVerified => ninFormatValid && nameMatchesNIN && nameMatchesProfile && issues.isEmpty;
...
class UserVerificationService {
  ...
  static VerificationResult verifyUserIdentity({
    ...
    if (profileName != null && nameOnIdPhoto != null) {
      ...
      nameSimilarityScore = NINValidator.calculateNameSimilarity(profileName, nameOnIdPhoto);
      
      if (!nameMatchesProfile) {
        if (nameSimilarityScore >= 0.8) {
          warnings.add('Names are similar but not exact match. Review required.');
        } else if (nameSimilarityScore >= 0.5) {
          issues.add('Names have partial match only. Verification required.');
        } else {
          issues.add('Names do not match. Profile name and ID photo name must be identical.');
        }
      }
    } else {
